...
@implementation UIColor (HexColors)
 static NSString *FLAG = @"UIColor+HexColors.m";
+(UIColor *)colorWithHexString:(NSString *)hexString {
    if ([hexString length] != 6) {
        [[LOLDebug sharedDebug] LogErrorContent:@"颜色字符串长度不正确" flag:FLAG line:16];
        return nil;
    }
    ...
    if (match != 0) {
        [[LOLDebug sharedDebug] LogErrorContent:@"颜色字符串字母不在十六进制范围内" flag:FLAG line:25];
        return nil;
    }
