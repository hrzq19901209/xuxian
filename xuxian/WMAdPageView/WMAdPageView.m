...
- (void)setAdsWithImages:(NSArray *)imageArray delegate:(id<WMAdPageViewDelegate>)delegate
{
...
- (void)tapAds
{
    if ([_delegate respondsToSelector:@selector(doTapOperationAtIndex:)]) {
        [_delegate doTapOperationAtIndex:_indexShow];
    }
}
...
- (void)reloadImages
{
    ...
    //lol
    [_imgPrev pin_setImageFromURL:[NSURL URLWithString:prevImage]];
    [_imgCurrent pin_setImageFromURL:[NSURL URLWithString:curImage]];
    [_imgNext pin_setImageFromURL:[NSURL URLWithString:nextImage]];
    /*-------------------------------end-------------------*/
